...
.service('RepeatParser', ['uiSelectMinErr', function(uiSelectMinErr) {
  ...
  self.parse = function(expression) {
    var match = expression.match(/^\s*([\s\S]+?)(?:\s+group\s+by\s+([\s\S]+?))?\s+in\s+([\s\S]+?)(?:\s+track\s+by\s+([\s\S]+?))?\s*$/);
  ...
  self.getNgRepeatExpression = function(lhs, rhs, trackByExp, groupByExp) {
    var expression = lhs + ' in ' + (groupByExp ? '$items' : rhs);
    if (trackByExp) {
...
.controller('uiSelectCtrl',
  ...
  ctrl.parseRepeatAttr = function(repeatAttr) {
    function updateGroups(items) {
      if(!ctrl.groups) {
        ...
        angular.forEach(ctrl.groups, function(group) {
          // fastest way to clear array http://jsperf.com/array-destroy/75
          while(group.length) { group.pop(); }
        });
      }
      angular.forEach(items, function(item) {
        var groupValue = item[repeat.groupByExp];
    ...
    var repeat = RepeatParser.parse(repeatAttr),
      setItemsFn = repeat.groupByExp ? updateGroups : setPlainItems;

    ctrl.isGrouped = repeat.groupByExp;
...
.directive('choices',
  ...
  return {
    ...
    compile: function(tElement, tAttrs) {
      ...
      choices.attr('ng-repeat', RepeatParser.getNgRepeatExpression(repeat.lhs, '$select.items', repeat.trackByExp, repeat.groupByExp))
...
angular.module('ui.select').run(['$templateCache', function ($templateCache) {
	$templateCache.put('bootstrap/choices.tpl.html', '<ul class="ui-select-choices ui-select-choices-content dropdown-menu" role="menu" aria-labelledby="dLabel" ng-show="$select.items.length> 0"> <li class="ui-select-choices-group"> <div class="divider" ng-show="$index> 0"></div> <div class="ui-select-choices-group-label dropdown-header">{{$group}}</div> <div class="ui-select-choices-row" ng-class="{active: $select.isActive(this)}"> <a href="javascript:void(0)" ng-transclude></a> </div> </li> </ul> ');
	...
	$templateCache.put('select2/choices.tpl.html', '<ul class="ui-select-choices ui-select-choices-content select2-results"> <li class="ui-select-choices-group"> <div class="ui-select-choices-group-label select2-result-label">{{$group}}</div> <ul class="select2-result-sub"> <li class="ui-select-choices-row" ng-class="{\'select2-highlighted\': $select.isActive(this)}"> <div class="select2-result-label" ng-transclude></div> </li> </ul> </li> </ul> ');
	...
	$templateCache.put('selectize/choices.tpl.html', '<div ng-show="$select.open" class="ui-select-choices selectize-dropdown single"> <div class="ui-select-choices-content selectize-dropdown-content"> <div class="ui-select-choices-group optgroup"> <div class="ui-select-choices-group-label optgroup-header">{{$group}}</div> <div class="ui-select-choices-row" ng-class="{active: $select.isActive(this)}"> <div class="option" data-selectable ng-transclude></div> </div> </div> </div> </div> ');
